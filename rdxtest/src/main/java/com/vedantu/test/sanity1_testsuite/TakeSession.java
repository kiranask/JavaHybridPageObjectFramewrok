...

{
	...
	public void TakingSession() throws Exception {
		{ // Get URL from the Excel Sheet
	/*		String xlPath = "./excel/BaseURL.xlsx";
			String Sheet = "Sheet1";
			String url = Excel.getCellValue(xlPath, Sheet, 1, 1);
			// Setting Hub

			URL hubURL = new URL("http://192.168.0.77:4444/wd/hub");
			DesiredCapabilities settings1 = new DesiredCapabilities();
			DesiredCapabilities settings2 = new DesiredCapabilities();*/
...
/*			settings1.setBrowserName("firefox");
			...
			// Sequential Launch
			driver1.get("https://fos-qa.vedantu.com/v/");
			driver2.get("https://fos-qa.vedantu.com/v/");
			// Login Student 

			driver1.findElement(By.xpath("(//a[contains(text(),'LOGIN')])[2]")).click();
		
			// WebDriverWait wait =new WebDriverWait(driver, 60);
			driver1.findElement(By.id("login-email")).sendKeys("qa-student@gmx.com");
			driver1.findElement(By.id("login-password")).sendKeys("vedantu123");
			...
			driver2.findElement(By.id("login-email")).sendKeys("qa-teacher@gmx.com");
			driver2.findElement(By.id("login-password")).sendKeys("vedantu123");
